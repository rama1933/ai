...
`backend/tests/test_sql_tool.py` was missed by this section's first draft and is the same kind of collateral: its positive tests used `chat_history` merely as a convenient allowlisted table, so removing the entry turns them red even though none of them is about conversation history. Retarget them at `documents`, which is what each was actually asserting.

**The allowlist alone does not hold, so the role's grant is removed too.** `tools/sql_tool.py` validates the query text, but a regex over SQL is not a security boundary. As shipped at `927c443` it was one pair of quote characters away from useless: `TABLE_REF` could not start at a `"`, so `SELECT session_id FROM "chat_history"` matched nothing, the allowlist ran over an empty list, and `rag_readonly` genuinely held `SELECT` on the table — the query executed and returned conversations. Verified directly against `_validate` (both `"chat_history"` and `"public"."chat_history"` passed) and against `has_table_privilege`.

The regex was tightened to accept and strip surrounding quotes, but the durable half is the grant, which is what PostgreSQL itself enforces. `db/schema.sql` now grants `rag_readonly` only on `documents`, and `db/migrations/002_sql_tool_readonly_scope.sql` revokes the old grant from databases that predate this change — a grant that predates a decision is exactly the kind that survives unnoticed. Decision 7 removes the last legitimate reason for that role to read conversations, so nothing is lost.

This also means `backend/tests/test_models.py`'s read-only probe now fails on the privilege rather than the foreign key. It asserts a broad `pytest.raises(Exception)`, so it still passes, but for a weaker reason. Recorded rather than widened.

**Deliberate change to the tool's self-description.** `backend/agent/registry.py:47-48` advertises `chat_history(id, session_id, role, message, created_at)` in the `sql_query` tool description. After Decision 7 that description is false, and the model acts on it: it emits `sql_query` for a question about conversation history, the tool refuses, and the user is answered with an apology for a question the assistant used to be able to answer. Removing the entry from the advertised column list is part of Decision 7 — the tool must not offer what it will not do. Verified by probe during Task 4: the old prompt now returns *"Maaf, saya tidak bisa menjalankan query pada tabel yang tidak diizinkan."* with no rows leaked, which is the correct security outcome and the wrong conversational one.
